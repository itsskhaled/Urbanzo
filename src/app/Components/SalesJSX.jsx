...
export default function SalesJSX() {
        ...
        <section ref={containerRef} className="min-h-screen w-full py-20">
            <div>
                <div className="flex w-full justify-center py-10">
                    <h1 ref={titleRef} className="text-5xl uppercase">sales</h1>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-[85%] gap-5 m-auto">
                    ...
                    {products.length === 0 ? (
                        ...
                        products.filter((product) => product.status === "sale")
                            .map((product, i) => {
                                return (
                                    <Link href={`New/${product.id}`}>
                                        <div className="relative h-100 overflow-hidden group cursor-pointer">
                                            {product.status === "sale" && (
                                                <p className="absolute py-0.5 px-2 rounded-sm text-white uppercase text-xs bg-red-500">{product.status}</p>
                                            )}
                                            <img
                                                src={product.image}
                                                alt=""
